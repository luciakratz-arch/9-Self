...
def gerar_laudo(tipo, asa_dominante, subtipo_dom, subtipo_int, subtipo_rem,
    ...
    # Imagem do tipo (01.jpg, 02.jpg...) após tipologia
    _img_dir = os.path.dirname(os.path.abspath(__file__))
    img_tipo_path = os.path.join(_img_dir, f'{tipo:02d}.jpg')
    if os.path.exists(img_tipo_path):
        try:
            story.append(sp(0.2))
            img_tipo = Image(img_tipo_path, width=10*cm, height=10*cm, kind='proportional')
            img_tipo.hAlign = 'CENTER'
            story.append(p(f'A imagem abaixo representa visualmente o tipo <b>{nome_tipo}</b> e suas dinâmicas de '
                           f'evolução e involução — ou seja, os caminhos de crescimento e de estresse que esse perfil pode '
                           f'percorrer ao longo da vida.'))
            story.append(sp(0.15))
    ...
    TEXTO_DINAMICA_P1 = (
        ...
        'significa que nenhum indivíduo opera isoladamente apenas em seu tipo principal; nossa '
        'estrutura psíquica é um sistema vivo, flexível e permeável, que transita continuamente '
        'entre diferentes estados de adaptação conforme as exigências do meio e o contexto '
        'emocional vivenciado.<br/><br/>'
        'Em resposta às demandas externas e aos estressores do dia a dia, essa mobilidade '
        'psicológica ocorre por caminhos internos bem definidos. O primeiro deles é o eixo de '
        'Estresse (ou desintegração), que é ativado quando as defesas habituais do sujeito se '
        'esgotam, revelando uma face mais automática, reativa e defensiva. Esse movimento funciona '
        'como um sinal de alerta e um mecanismo inconsciente de proteção frente à sobrecarga '
        'emocional.<br/><br/>'
        'Caminhando na direção oposta, a psique organiza-se em torno do eixo de Evolução (ou '
        'integração), que representa o norte do desenvolvimento pessoal. Este movimento demonstra '
        'a capacidade do indivíduo de acessar, de forma consciente e intencional, recursos de '
        'amadurecimento e virtudes que expandem sua percepção, permitindo respostas muito mais '
        'equilibradas, flexíveis e resilientes diante das circunstâncias, conforme ilustrado na '
        'figura abaixo.'
    )
    TEXTO_DINAMICA_P2 = (
    ...
    # Última página — fundo da marca com QR Code
    story.append(NextPageTemplate('Full'))
    story.append(PageBreak())
    story.append(Spacer(PW, PH))
